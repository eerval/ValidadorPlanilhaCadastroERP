...
// ATUALIZADO: Com lógica de Cadastro Novo vs Edição
// ==========================================================

// 1. Importar a biblioteca SheetJS
        ...
        return Object.entries(row)
            ...
            .some(([, value]) => {
                ...
                if (typeof value === 'string') {
                    ...
                    return trimmed !== '' && 
                           !['-', 'N/A', 'n/a', 'NULL', 'null', 'NaN'].includes(trimmed);
                }
...
// AGORA RECEBE O PARAMETRO 'MODE'
function runDataValidation(spreadsheetData, mode) {
    const results = [];
    ...
    spreadsheetData.forEach(row => {
        ...
        // ==========================================================
        // ID Interno - VALIDAÇÃO CONDICIONAL (NOVO/EDIÇÃO)
        // ==========================================================
        const idInternoValue = row['ID Interno'] ? row['ID Interno'].toString().trim() : '';

        if (mode === 'edicao') {
            // MODO EDIÇÃO: ID é OBRIGATÓRIO
            if (idInternoValue === '') {
                errors.push({ column: 'ID Interno', message: 'Para EDIÇÃO, o ID Interno é obrigatório.' });
            }
        } else {
            // MODO NOVO (Padrão): ID deve estar VAZIO
            if (idInternoValue !== '') {
                errors.push({ column: 'ID Interno', message: 'Para CADASTRO NOVO, o ID deve estar vazio.' });
            }
        }
...
self.onmessage = function(e) {
    ...
    if (action === 'PROCESS_FILE') {
        ...
        try {
            ...
            const originalRowCount = spreadsheetData.length;
            spreadsheetData = removeEmptyRows(spreadsheetData);
            const removedRowCount = originalRowCount - spreadsheetData.length;
            ...
            self.postMessage({ 
                ...
                removedEmptyRows: removedRowCount
            });
    ...
    // --- Validação de Dados (Etapa 3) ---
        ...
        // AQUI CAPTURAMOS O 'MODE' (NOVO OU EDICAO)
        ...
        function processBatch(startIndex) {
            ...
            // PASSAMOS O 'MODE' PARA A FUNÇÃO DE VALIDAÇÃO
            const { results: batchResults, correctedData: correctedBatchData } = runDataValidation(batchData, mode);
